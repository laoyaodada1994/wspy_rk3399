...
void parse_server(cJSON *param)
{
	...
	{
		if (strcmp(cJSON_GetArrayItem(param, i)->string, "port") == 0)
		{ //获取port
			UserCfgJson.port = cJSON_GetArrayItem(param, i)->valueint;
			printf("server port %d\n", UserCfgJson.port);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, "user") == 0)
		{ //获取user
			strcpy(UserCfgJson.clinet_id, cJSON_GetArrayItem(param, i)->valuestring);
			printf("client id  %s\n", UserCfgJson.clinet_id);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, "ip") == 0)
		{ //获取ip
...
void parse_device(cJSON *param)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (strcmp(cJSON_GetArrayItem(param, i)->string, "sn") == 0)
		{ //获取port
		...
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, LONGITUDE) == 0)
		{ //获取精度
			UserCfgJson.longitude = atof(cJSON_GetArrayItem(param, i)->valuestring);
			printf("longtitude %f\n", UserCfgJson.longitude);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, LATITUDE) == 0)
		{ //获取ip
...
void set_device(cJSON *param, char *setstr, float ftitude)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (strcmp(cJSON_GetArrayItem(param, i)->string, setstr) == 0)
		{ //获取port
...
void parse_ftp(cJSON *param)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (strcmp(cJSON_GetArrayItem(param, i)->string, "user") == 0)
		{ //获取port
			strcpy(UserCfgJson.user, cJSON_GetArrayItem(param, i)->valuestring);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, "passwd") == 0)
		{ //获取port
			strcpy(UserCfgJson.password, cJSON_GetArrayItem(param, i)->valuestring);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, "localpath") == 0)
		{ //获取port
			strcpy(UserCfgJson.localpath, cJSON_GetArrayItem(param, i)->valuestring);
		}
		else if (strcmp(cJSON_GetArrayItem(param, i)->string, "applocalpath") == 0)
		{ //获取port
...
void parse_gps(cJSON *param)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (strcmp(cJSON_GetArrayItem(param, i)->string, "disabled") == 0)
		{ //获取gps开关
...
void parse_wlan(cJSON *param)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (strcmp(cJSON_GetArrayItem(param, i)->string, dev_name) == 0)
		{ //获取网卡名称

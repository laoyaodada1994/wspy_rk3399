...
#define FIREMWARE_VERSION "v1.0.2"
...
void publish_routine(void) {
    ...
    while (1) {
        usleep(100);
        if (MQTT_Connc_On != MqttConnected) { continue; }
        now = time(NULL);
        if (now - pretick >= 3 || StatusQueryEvtOn) {
            status_report();
            gps_report();
            if (UserCfgJson.gps_disable == 0) {  // gps异常上报
                if (GPS_Data.latitude < 0.0001 || GPS_Data.longitude) { gps_abort(); }
            }
            pretick = now;
            printf("packet num: %d\t err num : %d\tbitrate :%f%%\n", PacketCount[0].totalcount, PacketCount[0].errcount,
                   ((float) PacketCount[0].errcount * 100) / PacketCount[0].totalcount);
            ...
            if (WifiAccess.mode == ACCESS_MODE_STA) {  // sta模式
                if (sta_acess_report(sockfd) == 0) {
                    ...
                    if (!StatusQueryEvtOn) { g_acesstimeout++; }
                }
...
void wlan_sniffer_publish(void) {
    while (PcapOn[0] == true || PcapOn[1] == true) {
        // print_wlan();
        if (PcapOn[0] == true) { sniffer_msg_push(20, 0); }
        if (PcapOn[1] == true) { sniffer_msg_push(20, 1); }
        usleep(1000);
    }
    pthread_exit(0);
}
...
void wspy_task(void) {
    ...
    for (;;) {
        if (PcapOn[IEEE80211_2G4] == true || PcapOn[IEEE80211_5G8] == true) {
            ...
            while (PcapOn[IEEE80211_2G4] == true || PcapOn[IEEE80211_5G8] == true) usleep(100);
            usleep(100);  //
#if 0
            ...
            if (ucch2 == IEEE80211_5G8) {
                ...
                if (pthflg[IEEE80211_5G8] == 0x1) {
                    printf("333\n");
                    if (chl1scan == false) { pthread_join(pth3, NULL); }
                    printf("444\n");
...
int main(int argc, char *argv[]) {
    ...
    int       res = 0;
    Log_Degug("Version: %s, Build: %s %s\n", FIREMWARE_VERSION, __DATE__, __TIME__);
    ...
    if (res <= 0) { gimbal_bsabort_send(0, 1); }
    res = gimbal_set_angle(0, 36);
    if (res <= 0) { gimbal_bsabort_send(0, 36); }
#else
